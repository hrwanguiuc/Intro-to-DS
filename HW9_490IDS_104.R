load("/Users/wanghaoran/Downloads/weather2011.rda")
    ...
    if(vertical){
      for(i in location){
        if(i == 31){
          ...
          segments(x0=i,y0=90,y1=28,lty=ltype)
        }
        else if(i==181 | i==212){
...
finalPlot = function(temp, precip){
  ...
  record_low_year = temp$LowYr
  record_high_year = temp$HighYr
  precip = temp$Precip
  record_precip = temp$RecordPrecip
  ...
  ### pdf("", width = , height = )
  ### layout(  )
  pdf("LA_weather_2011.pdf",width = 14, height = 7)
  ...
  text(high_days[which(high_value == max(high_value))]+0.5,
       max(high_value)+19,labels ="TIED RECORD", adj = 0)
  text(high_days[which(high_value == max(high_value))]+0.5,
       max(high_value)+15,labels = paste("HIGH:",max(high_value)), adj = 0)
  segments(x0=high_days[which(high_value == max(high_value))],y0=max(high_value),
           y1= max(high_value)+20)
  # for lowest tie
  ...
  #title(main = list("San Francisco's Weather in 2011", cex = 2), adj = 0)
  ...
  mtext( paste("Cumulative monthly precipitation in inches compared with normal monthly precipitation. Total precipitation in 2011 was", sum(precip), "inches."),
         line = 2, at=(180))
...
# 2 Research LifeCycles of This Case
#
# In the research lifecycle of the San Francisco's Weather case, we could see that
# the data is acquired from the website of National Weather Service Forecast Office
# website. Since it is obtained from an official source and the reliability of the 
# data is higher than other sources. So we could move to the next stage in our data
# lifecycle. In the second stage, we need to clean and organize our acquired data.
# For example, in this case, we need to convert some data into numeric formatt to 
# clarity whether the value is valid or not. Besides, there is some missing weather 
# data so we also need to label this kind of data in this process. In addition,
# some data maybe not helpful for our target of analysis, like the "Day" variable in
# the data frame.
#
# In the third stage, we need to model our data. In this case, we need to present the
# data in a time series line to see the trend of the temperature data. So we could 
# visualize our data and analyze the interesting property like trend, extreme value
# by the visualization of our data. In the fourth stage, after the visualization of 
# the weather data, we could publish our plot by generating a pdf file in this case.
# So we could share the knowledge we found in the visualization and it is easier for
# the readers to get the idea about the work we have done. Finally, the work and 
# visualization of our data could be preserved into file system for further use. For
# instance, others may want to replicate our work to verify the result and we may want
# to compare temperatures of different years in one graph and so on.

# Extra Credit
#
# 1. Design a plot that facilitates a comparison of San Francisco and Los Angeles weather.
# 
doublePlot = function(temp, precip, temp1, precip1){
  # The purpose of this function is to create the whole plot
  # Include here all of the set up that you need for
  # calling each of the above functions.
  # temp is the data frame sfoWeather or laxWeather
  ...
  ## LA DATA
  # another data frame
  totalPrecip1 = as.numeric(as.character(precip1$precip))
  normPrecip1 = as.numeric(as.character(precip1$normal))
  ### Fill in the various stages with your code
  ...
  obs_low1 = temp1$Low
  obs_high1 = temp1$High
  normal_low1 = temp1$NormalLow
  normal_high1 = temp1$NormalHigh
  record_low1 = temp1$RecordLow
  record_high1 = temp1$RecordHigh
  record_low_year1 = temp1$LowYr
  record_high_year1 = temp1$HighYr
  precip1 = temp1$Precip
  record_precip1 = temp1$RecordPrecip
  precip_year1 = temp1$PrecipYr
  # preprossing of the data
  record_precip1[is.na(record_precip1)] = 0
  precip1[is.na(precip1)] = 0
  # find which day has the highest/lowest temp(i.e. breaking days)
  high_value1 = obs_high1[which(obs_high1 == record_high1)]
  low_value1 = obs_low[which(obs_low1 == record_low1)]
  high_days1 = which(obs_high1 == record_high1)
  low_days1 =  which(obs_low1 == record_low1)
  
  ### Set up the graphics device to plot to pdf and layout
  ### the two plots on one canvas
  ### pdf("", width = , height = )
  ### layout(  )
  pdf("SF_LA_weather_2011.pdf",width = 14, height = 7)
  layout(c(1, 1, 1, 1, 1,2, 2))
  ...
  par(new=TRUE)
  drawTempRegion(1:365, obs_high1, obs_low1, col="#C0FF3E")
  ...
  ### Add the titles 
  #title(main = list("Los Angeles's Weather in 2011", cex = 2), adj = 0)
  title(main = list("Comparison between SF and LA Weather in 2011", cex = 2), adj = 0)
  text(3, ceiling(max(record_high/10))*10-2, labels = "Temperature", 
       cex = 1.5, font = 2, adj=0)
  text(3, ceiling(max(record_high/10))*10 - 7, 
       labels = "Bars represent range between ", cex = 1.5, adj = 0)
  ...
  addGrid(cumDays[c(-1,-length(cumDays))]-1,"#000000",3)
  # add horizontal white ref line
  addGrid(1:y_precip_highest, "#FFE4C4", 1, vertical = FALSE)
  for(i in 1:12){
    days_in_month = daysInMonth[i]
    precip_index = cumDays[i]:(cumDays[i+1]-1)
    monthPrecip(precip_index,precip[precip_index],normal = normPrecip[i])
...
#doublePlot(temp = sfoWeather, precip = sfoMonthlyPrecip,temp1 = laxWeather, precip1 = laxMonthlyPrecip)

## Extra Credit 
## 2. Plot SF weather in 2015

dataset = read.csv("/Users/wanghaoran/Downloads/2015Data.csv", header = TRUE)

extra_finalPlot = function(temp,precip){
  # The purpose of this function is to create the whole plot
  # Include here all of the set up that you need for
  # calling each of the above functions.
  # temp is the data frame sfoWeather or laxWeather
...
### Call: finalPlot(temp = sfoWeather, precip = sfoMonthlyPrecip)
# extra_finalPlot(temp = dataset,precip = sfoMonthlyPrecip)
